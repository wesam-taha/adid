...
class caudittrail_list extends caudittrail {
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_datetime") && $objForm->HasValue("o_datetime") && $this->datetime->CurrentValue <> $this->datetime->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_script") && $objForm->HasValue("o_script") && $this->script->CurrentValue <> $this->script->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_user") && $objForm->HasValue("o_user") && $this->user->CurrentValue <> $this->user->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_action") && $objForm->HasValue("o_action") && $this->action->CurrentValue <> $this->action->OldValue)
			return FALSE;
		if ($objForm->HasValue("x__table") && $objForm->HasValue("o__table") && $this->_table->CurrentValue <> $this->_table->OldValue)
			return FALSE;
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewAction ewMultiDelete\" title=\"" . ew_HtmlTitle($Language->Phrase("DeleteSelectedLink")) . "\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("DeleteSelectedLink")) . "\" href=\"\" onclick=\"ew_SubmitAction(event,{f:document.faudittraillist,url:'" . $this->MultiDeleteUrl . "'});return false;\">" . $Language->Phrase("DeleteSelectedLink") . "</a>";
		$item->Visible = ($Security->CanDelete());
	...
	function SetupSearchOptions() {
		...
		if (ew_IsMobile())
			...
			$item->Body = "<button type=\"button\" class=\"btn btn-default ewAdvancedSearch\" title=\"" . $Language->Phrase("AdvancedSearch") . "\" data-table=\"audittrail\" data-caption=\"" . $Language->Phrase("AdvancedSearch") . "\" onclick=\"ew_ModalDialogShow({lnk:this,btn:'SearchBtn',url:'audittrailsrch.php'});\">" . $Language->Phrase("AdvancedSearchBtn") . "</button>";
		$item->Visible = TRUE;
	...
	function LoadSearchValues() {
		...
		// script
		$this->script->AdvancedSearch->SearchValue = @$_GET["x_script"];
		if ($this->script->AdvancedSearch->SearchValue <> "" && $this->Command == "") $this->Command = "search";
		...
		// action
		$this->action->AdvancedSearch->SearchValue = @$_GET["x_action"];
		if ($this->action->AdvancedSearch->SearchValue <> "" && $this->Command == "") $this->Command = "search";
		$this->action->AdvancedSearch->SearchOperator = @$_GET["z_action"];

		// table
		$this->_table->AdvancedSearch->SearchValue = @$_GET["x__table"];
		if ($this->_table->AdvancedSearch->SearchValue <> "" && $this->Command == "") $this->Command = "search";
		$this->_table->AdvancedSearch->SearchOperator = @$_GET["z__table"];

		// field
		$this->_field->AdvancedSearch->SearchValue = @$_GET["x__field"];
		if ($this->_field->AdvancedSearch->SearchValue <> "" && $this->Command == "") $this->Command = "search";
		$this->_field->AdvancedSearch->SearchOperator = @$_GET["z__field"];

		// keyvalue
		$this->keyvalue->AdvancedSearch->SearchValue = @$_GET["x_keyvalue"];
		if ($this->keyvalue->AdvancedSearch->SearchValue <> "" && $this->Command == "") $this->Command = "search";
		$this->keyvalue->AdvancedSearch->SearchOperator = @$_GET["z_keyvalue"];

		// oldvalue
		$this->oldvalue->AdvancedSearch->SearchValue = @$_GET["x_oldvalue"];
		if ($this->oldvalue->AdvancedSearch->SearchValue <> "" && $this->Command == "") $this->Command = "search";
		$this->oldvalue->AdvancedSearch->SearchOperator = @$_GET["z_oldvalue"];

		// newvalue
		$this->newvalue->AdvancedSearch->SearchValue = @$_GET["x_newvalue"];
		if ($this->newvalue->AdvancedSearch->SearchValue <> "" && $this->Command == "") $this->Command = "search";
	...
	function LoadFormValues() {
		...
		$this->datetime->setOldValue($objForm->GetValue("o_datetime"));
		if (!$this->script->FldIsDetailKey) {
			$this->script->setFormValue($objForm->GetValue("x_script"));
		}
		$this->script->setOldValue($objForm->GetValue("o_script"));
		...
		$this->action->setOldValue($objForm->GetValue("o_action"));
		if (!$this->_table->FldIsDetailKey) {
			$this->_table->setFormValue($objForm->GetValue("x__table"));
		}
		$this->_table->setOldValue($objForm->GetValue("o__table"));
...
// Validate form
faudittraillist.Validate = function() {
...
// Check empty row
faudittraillist.EmptyRow = function(infix) {
	var fobj = this.Form;
	if (ew_ValueChanged(fobj, infix, "datetime", false)) return false;
	if (ew_ValueChanged(fobj, infix, "script", false)) return false;
	if (ew_ValueChanged(fobj, infix, "user", false)) return false;
	if (ew_ValueChanged(fobj, infix, "action", false)) return false;
	if (ew_ValueChanged(fobj, infix, "_table", false)) return false;
...
if ($audittrail->CurrentAction == "gridadd") {
	$audittrail->CurrentFilter = "0=1";
	$audittrail_list->StartRec = 1;
	$audittrail_list->DisplayRecs = $audittrail->GridAddRowCount;
	$audittrail_list->TotalRecs = $audittrail_list->DisplayRecs;
	$audittrail_list->StopRec = $audittrail_list->DisplayRecs;
} else {
...
if ($objForm) {
	$objForm->Index = -1;
	if ($objForm->HasValue($audittrail_list->FormKeyCountName) && ($audittrail->CurrentAction == "gridadd" || $audittrail->CurrentAction == "gridedit" || $audittrail->CurrentAction == "F")) {
		$audittrail_list->KeyCount = $objForm->GetValue($audittrail_list->FormKeyCountName);
		$audittrail_list->StopRec = $audittrail_list->StartRec + $audittrail_list->KeyCount - 1;
	}
}
...
if ($audittrail->CurrentAction == "gridadd")
	$audittrail_list->RowIndex = 0;
if ($audittrail->CurrentAction == "gridedit")
	$audittrail_list->RowIndex = 0;
while ($audittrail_list->RecCnt < $audittrail_list->StopRec) {
	...
	if (intval($audittrail_list->RecCnt) >= intval($audittrail_list->StartRec)) {
		...
		if ($audittrail->CurrentAction == "gridadd" || $audittrail->CurrentAction == "gridedit" || $audittrail->CurrentAction == "F") {
			$audittrail_list->RowIndex++;
			$objForm->Index = $audittrail_list->RowIndex;
			if ($objForm->HasValue($audittrail_list->FormActionName))
				$audittrail_list->RowAction = strval($objForm->GetValue($audittrail_list->FormActionName));
			elseif ($audittrail->CurrentAction == "gridadd")
				$audittrail_list->RowAction = "insert";
			else
				$audittrail_list->RowAction = "";
		}
		...
		if ($audittrail->CurrentAction == "gridadd") // Grid add
			$audittrail->RowType = EW_ROWTYPE_ADD; // Render add
		if ($audittrail->CurrentAction == "gridadd" && $audittrail->EventCancelled && !$objForm->HasValue("k_blankrow")) // Insert failed
			$audittrail_list->RestoreCurrentRowFormValues($audittrail_list->RowIndex); // Restore form values
		if ($audittrail->CurrentAction == "gridedit") { // Grid edit
			if ($audittrail->EventCancelled) {
				$audittrail_list->RestoreCurrentRowFormValues($audittrail_list->RowIndex); // Restore form values
			}
			if ($audittrail_list->RowAction == "insert")
				$audittrail->RowType = EW_ROWTYPE_ADD; // Render add
			else
				$audittrail->RowType = EW_ROWTYPE_EDIT; // Render edit
		}
		if ($audittrail->CurrentAction == "gridedit" && ($audittrail->RowType == EW_ROWTYPE_EDIT || $audittrail->RowType == EW_ROWTYPE_ADD) && $audittrail->EventCancelled) // Update failed
			$audittrail_list->RestoreCurrentRowFormValues($audittrail_list->RowIndex); // Restore form values
		if ($audittrail->RowType == EW_ROWTYPE_EDIT) // Edit row
			$audittrail_list->EditRowCnt++;
		...
		if ($audittrail_list->RowAction <> "delete" && $audittrail_list->RowAction <> "insertdelete" && !($audittrail_list->RowAction == "insert" && $audittrail->CurrentAction == "F" && $audittrail_list->EmptyRow())) {
?>
...
<?php if ($audittrail->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<input type="hidden" data-table="audittrail" data-field="x_id" name="o<?php echo $audittrail_list->RowIndex ?>_id" id="o<?php echo $audittrail_list->RowIndex ?>_id" value="<?php echo ew_HtmlEncode($audittrail->id->OldValue) ?>">
<?php } ?>
<?php if ($audittrail->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<span id="el<?php echo $audittrail_list->RowCnt ?>_audittrail_id" class="form-group audittrail_id">
<span<?php echo $audittrail->id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $audittrail->id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="audittrail" data-field="x_id" name="x<?php echo $audittrail_list->RowIndex ?>_id" id="x<?php echo $audittrail_list->RowIndex ?>_id" value="<?php echo ew_HtmlEncode($audittrail->id->CurrentValue) ?>">
<?php } ?>
<?php if ($audittrail->RowType == EW_ROWTYPE_VIEW) { // View record ?>
...
<?php if ($audittrail->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<span id="el<?php echo $audittrail_list->RowCnt ?>_audittrail_datetime" class="form-group audittrail_datetime">
<input type="text" data-table="audittrail" data-field="x_datetime" name="x<?php echo $audittrail_list->RowIndex ?>_datetime" id="x<?php echo $audittrail_list->RowIndex ?>_datetime" placeholder="<?php echo ew_HtmlEncode($audittrail->datetime->getPlaceHolder()) ?>" value="<?php echo $audittrail->datetime->EditValue ?>"<?php echo $audittrail->datetime->EditAttributes() ?>>
...
<script type="text/javascript">
ew_CreateDateTimePicker("faudittraillist", "x<?php echo $audittrail_list->RowIndex ?>_datetime", {"ignoreReadonly":true,"useCurrent":false,"format":0});
</script>
<?php } ?>
</span>
<input type="hidden" data-table="audittrail" data-field="x_datetime" name="o<?php echo $audittrail_list->RowIndex ?>_datetime" id="o<?php echo $audittrail_list->RowIndex ?>_datetime" value="<?php echo ew_HtmlEncode($audittrail->datetime->OldValue) ?>">
<?php } ?>
<?php if ($audittrail->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<span id="el<?php echo $audittrail_list->RowCnt ?>_audittrail_datetime" class="form-group audittrail_datetime">
<input type="text" data-table="audittrail" data-field="x_datetime" name="x<?php echo $audittrail_list->RowIndex ?>_datetime" id="x<?php echo $audittrail_list->RowIndex ?>_datetime" placeholder="<?php echo ew_HtmlEncode($audittrail->datetime->getPlaceHolder()) ?>" value="<?php echo $audittrail->datetime->EditValue ?>"<?php echo $audittrail->datetime->EditAttributes() ?>>
...
<?php if ($audittrail->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<span id="el<?php echo $audittrail_list->RowCnt ?>_audittrail_script" class="form-group audittrail_script">
<input type="text" data-table="audittrail" data-field="x_script" name="x<?php echo $audittrail_list->RowIndex ?>_script" id="x<?php echo $audittrail_list->RowIndex ?>_script" size="30" maxlength="255" placeholder="<?php echo ew_HtmlEncode($audittrail->script->getPlaceHolder()) ?>" value="<?php echo $audittrail->script->EditValue ?>"<?php echo $audittrail->script->EditAttributes() ?>>
</span>
<input type="hidden" data-table="audittrail" data-field="x_script" name="o<?php echo $audittrail_list->RowIndex ?>_script" id="o<?php echo $audittrail_list->RowIndex ?>_script" value="<?php echo ew_HtmlEncode($audittrail->script->OldValue) ?>">
<?php } ?>
<?php if ($audittrail->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<span id="el<?php echo $audittrail_list->RowCnt ?>_audittrail_script" class="form-group audittrail_script">
<input type="text" data-table="audittrail" data-field="x_script" name="x<?php echo $audittrail_list->RowIndex ?>_script" id="x<?php echo $audittrail_list->RowIndex ?>_script" size="30" maxlength="255" placeholder="<?php echo ew_HtmlEncode($audittrail->script->getPlaceHolder()) ?>" value="<?php echo $audittrail->script->EditValue ?>"<?php echo $audittrail->script->EditAttributes() ?>>
...
<?php if ($audittrail->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<span id="el<?php echo $audittrail_list->RowCnt ?>_audittrail_user" class="form-group audittrail_user">
<input type="text" data-table="audittrail" data-field="x_user" name="x<?php echo $audittrail_list->RowIndex ?>_user" id="x<?php echo $audittrail_list->RowIndex ?>_user" size="30" maxlength="255" placeholder="<?php echo ew_HtmlEncode($audittrail->user->getPlaceHolder()) ?>" value="<?php echo $audittrail->user->EditValue ?>"<?php echo $audittrail->user->EditAttributes() ?>>
</span>
<input type="hidden" data-table="audittrail" data-field="x_user" name="o<?php echo $audittrail_list->RowIndex ?>_user" id="o<?php echo $audittrail_list->RowIndex ?>_user" value="<?php echo ew_HtmlEncode($audittrail->user->OldValue) ?>">
<?php } ?>
<?php if ($audittrail->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<span id="el<?php echo $audittrail_list->RowCnt ?>_audittrail_user" class="form-group audittrail_user">
<input type="text" data-table="audittrail" data-field="x_user" name="x<?php echo $audittrail_list->RowIndex ?>_user" id="x<?php echo $audittrail_list->RowIndex ?>_user" size="30" maxlength="255" placeholder="<?php echo ew_HtmlEncode($audittrail->user->getPlaceHolder()) ?>" value="<?php echo $audittrail->user->EditValue ?>"<?php echo $audittrail->user->EditAttributes() ?>>
...
<?php if ($audittrail->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<span id="el<?php echo $audittrail_list->RowCnt ?>_audittrail_action" class="form-group audittrail_action">
<input type="text" data-table="audittrail" data-field="x_action" name="x<?php echo $audittrail_list->RowIndex ?>_action" id="x<?php echo $audittrail_list->RowIndex ?>_action" size="30" maxlength="255" placeholder="<?php echo ew_HtmlEncode($audittrail->action->getPlaceHolder()) ?>" value="<?php echo $audittrail->action->EditValue ?>"<?php echo $audittrail->action->EditAttributes() ?>>
</span>
<input type="hidden" data-table="audittrail" data-field="x_action" name="o<?php echo $audittrail_list->RowIndex ?>_action" id="o<?php echo $audittrail_list->RowIndex ?>_action" value="<?php echo ew_HtmlEncode($audittrail->action->OldValue) ?>">
<?php } ?>
<?php if ($audittrail->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<span id="el<?php echo $audittrail_list->RowCnt ?>_audittrail_action" class="form-group audittrail_action">
<input type="text" data-table="audittrail" data-field="x_action" name="x<?php echo $audittrail_list->RowIndex ?>_action" id="x<?php echo $audittrail_list->RowIndex ?>_action" size="30" maxlength="255" placeholder="<?php echo ew_HtmlEncode($audittrail->action->getPlaceHolder()) ?>" value="<?php echo $audittrail->action->EditValue ?>"<?php echo $audittrail->action->EditAttributes() ?>>
...
<?php if ($audittrail->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<span id="el<?php echo $audittrail_list->RowCnt ?>_audittrail__table" class="form-group audittrail__table">
<input type="text" data-table="audittrail" data-field="x__table" name="x<?php echo $audittrail_list->RowIndex ?>__table" id="x<?php echo $audittrail_list->RowIndex ?>__table" size="30" maxlength="255" placeholder="<?php echo ew_HtmlEncode($audittrail->_table->getPlaceHolder()) ?>" value="<?php echo $audittrail->_table->EditValue ?>"<?php echo $audittrail->_table->EditAttributes() ?>>
</span>
<input type="hidden" data-table="audittrail" data-field="x__table" name="o<?php echo $audittrail_list->RowIndex ?>__table" id="o<?php echo $audittrail_list->RowIndex ?>__table" value="<?php echo ew_HtmlEncode($audittrail->_table->OldValue) ?>">
<?php } ?>
<?php if ($audittrail->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<span id="el<?php echo $audittrail_list->RowCnt ?>_audittrail__table" class="form-group audittrail__table">
<input type="text" data-table="audittrail" data-field="x__table" name="x<?php echo $audittrail_list->RowIndex ?>__table" id="x<?php echo $audittrail_list->RowIndex ?>__table" size="30" maxlength="255" placeholder="<?php echo ew_HtmlEncode($audittrail->_table->getPlaceHolder()) ?>" value="<?php echo $audittrail->_table->EditValue ?>"<?php echo $audittrail->_table->EditAttributes() ?>>
...
<?php if ($audittrail->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<span id="el<?php echo $audittrail_list->RowCnt ?>_audittrail__field" class="form-group audittrail__field">
<input type="text" data-table="audittrail" data-field="x__field" name="x<?php echo $audittrail_list->RowIndex ?>__field" id="x<?php echo $audittrail_list->RowIndex ?>__field" size="30" maxlength="255" placeholder="<?php echo ew_HtmlEncode($audittrail->_field->getPlaceHolder()) ?>" value="<?php echo $audittrail->_field->EditValue ?>"<?php echo $audittrail->_field->EditAttributes() ?>>
</span>
<input type="hidden" data-table="audittrail" data-field="x__field" name="o<?php echo $audittrail_list->RowIndex ?>__field" id="o<?php echo $audittrail_list->RowIndex ?>__field" value="<?php echo ew_HtmlEncode($audittrail->_field->OldValue) ?>">
<?php } ?>
<?php if ($audittrail->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<span id="el<?php echo $audittrail_list->RowCnt ?>_audittrail__field" class="form-group audittrail__field">
<input type="text" data-table="audittrail" data-field="x__field" name="x<?php echo $audittrail_list->RowIndex ?>__field" id="x<?php echo $audittrail_list->RowIndex ?>__field" size="30" maxlength="255" placeholder="<?php echo ew_HtmlEncode($audittrail->_field->getPlaceHolder()) ?>" value="<?php echo $audittrail->_field->EditValue ?>"<?php echo $audittrail->_field->EditAttributes() ?>>
...
<?php if ($audittrail->RowType == EW_ROWTYPE_ADD || $audittrail->RowType == EW_ROWTYPE_EDIT) { ?>
<script type="text/javascript">
faudittraillist.UpdateOpts(<?php echo $audittrail_list->RowIndex ?>);
</script>
<?php } ?>
<?php
	}
	...
	if ($audittrail->CurrentAction <> "gridadd")
		if (!$audittrail_list->Recordset->EOF) $audittrail_list->Recordset->MoveNext();
}
?>
<?php
	if ($audittrail->CurrentAction == "gridadd" || $audittrail->CurrentAction == "gridedit") {
		$audittrail_list->RowIndex = '$rowindex$';
		$audittrail_list->LoadDefaultValues();

		// Set row properties
		$audittrail->ResetAttrs();
		$audittrail->RowAttrs = array_merge($audittrail->RowAttrs, array('data-rowindex'=>$audittrail_list->RowIndex, 'id'=>'r0_audittrail', 'data-rowtype'=>EW_ROWTYPE_ADD));
		ew_AppendClass($audittrail->RowAttrs["class"], "ewTemplate");
		$audittrail->RowType = EW_ROWTYPE_ADD;
		...
		// Render list options
		$audittrail_list->RenderListOptions();
		$audittrail_list->StartRowCnt = 0;
?>
	<tr<?php echo $audittrail->RowAttributes() ?>>
<?php

// Render list options (body, left)
$audittrail_list->ListOptions->Render("body", "left", $audittrail_list->RowIndex);
?>
	<?php if ($audittrail->id->Visible) { // id ?>
		<td data-name="id">
<input type="hidden" data-table="audittrail" data-field="x_id" name="o<?php echo $audittrail_list->RowIndex ?>_id" id="o<?php echo $audittrail_list->RowIndex ?>_id" value="<?php echo ew_HtmlEncode($audittrail->id->OldValue) ?>">
...
<span id="el$rowindex$_audittrail_datetime" class="form-group audittrail_datetime">
<input type="text" data-table="audittrail" data-field="x_datetime" name="x<?php echo $audittrail_list->RowIndex ?>_datetime" id="x<?php echo $audittrail_list->RowIndex ?>_datetime" placeholder="<?php echo ew_HtmlEncode($audittrail->datetime->getPlaceHolder()) ?>" value="<?php echo $audittrail->datetime->EditValue ?>"<?php echo $audittrail->datetime->EditAttributes() ?>>
<?php if (!$audittrail->datetime->ReadOnly && !$audittrail->datetime->Disabled && !isset($audittrail->datetime->EditAttrs["readonly"]) && !isset($audittrail->datetime->EditAttrs["disabled"])) { ?>
<script type="text/javascript">
ew_CreateDateTimePicker("faudittraillist", "x<?php echo $audittrail_list->RowIndex ?>_datetime", {"ignoreReadonly":true,"useCurrent":false,"format":0});
...
<input type="hidden" data-table="audittrail" data-field="x_datetime" name="o<?php echo $audittrail_list->RowIndex ?>_datetime" id="o<?php echo $audittrail_list->RowIndex ?>_datetime" value="<?php echo ew_HtmlEncode($audittrail->datetime->OldValue) ?>">
	...
	<?php if ($audittrail->script->Visible) { // script ?>
		<td data-name="script">
<span id="el$rowindex$_audittrail_script" class="form-group audittrail_script">
<input type="text" data-table="audittrail" data-field="x_script" name="x<?php echo $audittrail_list->RowIndex ?>_script" id="x<?php echo $audittrail_list->RowIndex ?>_script" size="30" maxlength="255" placeholder="<?php echo ew_HtmlEncode($audittrail->script->getPlaceHolder()) ?>" value="<?php echo $audittrail->script->EditValue ?>"<?php echo $audittrail->script->EditAttributes() ?>>
</span>
<input type="hidden" data-table="audittrail" data-field="x_script" name="o<?php echo $audittrail_list->RowIndex ?>_script" id="o<?php echo $audittrail_list->RowIndex ?>_script" value="<?php echo ew_HtmlEncode($audittrail->script->OldValue) ?>">
</td>
	...
	<?php if ($audittrail->user->Visible) { // user ?>
		<td data-name="user">
<span id="el$rowindex$_audittrail_user" class="form-group audittrail_user">
<input type="text" data-table="audittrail" data-field="x_user" name="x<?php echo $audittrail_list->RowIndex ?>_user" id="x<?php echo $audittrail_list->RowIndex ?>_user" size="30" maxlength="255" placeholder="<?php echo ew_HtmlEncode($audittrail->user->getPlaceHolder()) ?>" value="<?php echo $audittrail->user->EditValue ?>"<?php echo $audittrail->user->EditAttributes() ?>>
</span>
<input type="hidden" data-table="audittrail" data-field="x_user" name="o<?php echo $audittrail_list->RowIndex ?>_user" id="o<?php echo $audittrail_list->RowIndex ?>_user" value="<?php echo ew_HtmlEncode($audittrail->user->OldValue) ?>">
</td>
	...
	<?php if ($audittrail->action->Visible) { // action ?>
		<td data-name="action">
<span id="el$rowindex$_audittrail_action" class="form-group audittrail_action">
<input type="text" data-table="audittrail" data-field="x_action" name="x<?php echo $audittrail_list->RowIndex ?>_action" id="x<?php echo $audittrail_list->RowIndex ?>_action" size="30" maxlength="255" placeholder="<?php echo ew_HtmlEncode($audittrail->action->getPlaceHolder()) ?>" value="<?php echo $audittrail->action->EditValue ?>"<?php echo $audittrail->action->EditAttributes() ?>>
</span>
<input type="hidden" data-table="audittrail" data-field="x_action" name="o<?php echo $audittrail_list->RowIndex ?>_action" id="o<?php echo $audittrail_list->RowIndex ?>_action" value="<?php echo ew_HtmlEncode($audittrail->action->OldValue) ?>">
</td>
	...
	<?php if ($audittrail->_table->Visible) { // table ?>
		<td data-name="_table">
<span id="el$rowindex$_audittrail__table" class="form-group audittrail__table">
<input type="text" data-table="audittrail" data-field="x__table" name="x<?php echo $audittrail_list->RowIndex ?>__table" id="x<?php echo $audittrail_list->RowIndex ?>__table" size="30" maxlength="255" placeholder="<?php echo ew_HtmlEncode($audittrail->_table->getPlaceHolder()) ?>" value="<?php echo $audittrail->_table->EditValue ?>"<?php echo $audittrail->_table->EditAttributes() ?>>
</span>
<input type="hidden" data-table="audittrail" data-field="x__table" name="o<?php echo $audittrail_list->RowIndex ?>__table" id="o<?php echo $audittrail_list->RowIndex ?>__table" value="<?php echo ew_HtmlEncode($audittrail->_table->OldValue) ?>">
</td>
	...
	<?php if ($audittrail->_field->Visible) { // field ?>
		<td data-name="_field">
<span id="el$rowindex$_audittrail__field" class="form-group audittrail__field">
<input type="text" data-table="audittrail" data-field="x__field" name="x<?php echo $audittrail_list->RowIndex ?>__field" id="x<?php echo $audittrail_list->RowIndex ?>__field" size="30" maxlength="255" placeholder="<?php echo ew_HtmlEncode($audittrail->_field->getPlaceHolder()) ?>" value="<?php echo $audittrail->_field->EditValue ?>"<?php echo $audittrail->_field->EditAttributes() ?>>
</span>
<input type="hidden" data-table="audittrail" data-field="x__field" name="o<?php echo $audittrail_list->RowIndex ?>__field" id="o<?php echo $audittrail_list->RowIndex ?>__field" value="<?php echo ew_HtmlEncode($audittrail->_field->OldValue) ?>">
</td>
...
<?php } ?>
<?php if ($audittrail->CurrentAction == "gridadd") { ?>
<input type="hidden" name="a_list" id="a_list" value="gridinsert">
<input type="hidden" name="<?php echo $audittrail_list->FormKeyCountName ?>" id="<?php echo $audittrail_list->FormKeyCountName ?>" value="<?php echo $audittrail_list->KeyCount ?>">
<?php echo $audittrail_list->MultiSelectKey ?>
<?php } ?>
<?php if ($audittrail->CurrentAction == "gridedit") { ?>
