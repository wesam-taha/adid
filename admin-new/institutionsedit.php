...
$institutions_edit = NULL; // Initialize page object first

class cinstitutions_edit extends cinstitutions {
	...
	// Page object name
	var $PageObjName = 'institutions_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->institution_id->CurrentValue == "") {
			$this->Page_Terminate("institutionslist.php"); // Invalid key, return to list
		}
	...
	function LoadFormValues() {
		...
		if (!$this->institution_id->FldIsDetailKey)
			$this->institution_id->setFormValue($objForm->GetValue("x_institution_id"));
	...
	function RestoreFormValues() {
		global $objForm;
		$this->LoadRow();
		$this->institution_id->CurrentValue = $this->institution_id->FormValue;
	...
	function RenderRow() {
		...
		$this->security_comment->ViewCustomAttributes = "";
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->trade_licence);
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->visa_copy);
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->emirates_id_copy);
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->passport_copy);
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// full_name_ar
			$this->full_name_ar->SetDbValueDef($rsnew, $this->full_name_ar->CurrentValue, NULL, $this->full_name_ar->ReadOnly);

			// full_name_en
			$this->full_name_en->SetDbValueDef($rsnew, $this->full_name_en->CurrentValue, NULL, $this->full_name_en->ReadOnly);

			// institution_type
			$this->institution_type->SetDbValueDef($rsnew, $this->institution_type->CurrentValue, NULL, $this->institution_type->ReadOnly);

			// institutes_name
			$this->institutes_name->SetDbValueDef($rsnew, $this->institutes_name->CurrentValue, NULL, $this->institutes_name->ReadOnly);

			// volunteering_type
			$this->volunteering_type->SetDbValueDef($rsnew, $this->volunteering_type->CurrentValue, NULL, $this->volunteering_type->ReadOnly);

			// licence_no
			$this->licence_no->SetDbValueDef($rsnew, $this->licence_no->CurrentValue, NULL, $this->licence_no->ReadOnly);

			// trade_licence
			if ($this->trade_licence->Visible && !$this->trade_licence->ReadOnly && !$this->trade_licence->Upload->KeepFile) {
				$this->trade_licence->Upload->DbValue = $rsold['trade_licence']; // Get original value
			...
			// tl_expiry_date
			$this->tl_expiry_date->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->tl_expiry_date->CurrentValue, 0), NULL, $this->tl_expiry_date->ReadOnly);

			// nationality_type
			$this->nationality_type->SetDbValueDef($rsnew, $this->nationality_type->CurrentValue, "", $this->nationality_type->ReadOnly);

			// nationality
			$this->nationality->SetDbValueDef($rsnew, $this->nationality->CurrentValue, NULL, $this->nationality->ReadOnly);

			// visa_expiry_date
			$this->visa_expiry_date->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->visa_expiry_date->CurrentValue, 0), ew_CurrentDate(), $this->visa_expiry_date->ReadOnly);

			// unid
			$this->unid->SetDbValueDef($rsnew, $this->unid->CurrentValue, 0, $this->unid->ReadOnly);

			// visa_copy
			if ($this->visa_copy->Visible && !$this->visa_copy->ReadOnly && !$this->visa_copy->Upload->KeepFile) {
				$this->visa_copy->Upload->DbValue = $rsold['visa_copy']; // Get original value
			...
			// current_emirate
			$this->current_emirate->SetDbValueDef($rsnew, $this->current_emirate->CurrentValue, NULL, $this->current_emirate->ReadOnly);

			// full_address
			$this->full_address->SetDbValueDef($rsnew, $this->full_address->CurrentValue, NULL, $this->full_address->ReadOnly);

			// emirates_id_number
			$this->emirates_id_number->SetDbValueDef($rsnew, $this->emirates_id_number->CurrentValue, NULL, $this->emirates_id_number->ReadOnly);

			// eid_expiry_date
			$this->eid_expiry_date->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->eid_expiry_date->CurrentValue, 0), NULL, $this->eid_expiry_date->ReadOnly);

			// emirates_id_copy
			if ($this->emirates_id_copy->Visible && !$this->emirates_id_copy->ReadOnly && !$this->emirates_id_copy->Upload->KeepFile) {
				$this->emirates_id_copy->Upload->DbValue = $rsold['emirates_id_copy']; // Get original value
			...
			// passport_number
			$this->passport_number->SetDbValueDef($rsnew, $this->passport_number->CurrentValue, NULL, $this->passport_number->ReadOnly);

			// passport_ex_date
			$this->passport_ex_date->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->passport_ex_date->CurrentValue, 0), NULL, $this->passport_ex_date->ReadOnly);

			// passport_copy
			if ($this->passport_copy->Visible && !$this->passport_copy->ReadOnly && !$this->passport_copy->Upload->KeepFile) {
				$this->passport_copy->Upload->DbValue = $rsold['passport_copy']; // Get original value
			...
			// place_of_work
			$this->place_of_work->SetDbValueDef($rsnew, $this->place_of_work->CurrentValue, NULL, $this->place_of_work->ReadOnly);

			// work_phone
			$this->work_phone->SetDbValueDef($rsnew, $this->work_phone->CurrentValue, NULL, $this->work_phone->ReadOnly);
			...
			// pobbox
			$this->pobbox->SetDbValueDef($rsnew, $this->pobbox->CurrentValue, NULL, $this->pobbox->ReadOnly);
			...
			// admin_approval
			$this->admin_approval->SetDbValueDef($rsnew, $this->admin_approval->CurrentValue, NULL, $this->admin_approval->ReadOnly);

			// admin_comment
			$this->admin_comment->SetDbValueDef($rsnew, $this->admin_comment->CurrentValue, NULL, $this->admin_comment->ReadOnly);

			// forward_to_dep
			$this->forward_to_dep->SetDbValueDef($rsnew, $this->forward_to_dep->CurrentValue, NULL, $this->forward_to_dep->ReadOnly);

			// eco_department_approval
			$this->eco_department_approval->SetDbValueDef($rsnew, $this->eco_department_approval->CurrentValue, NULL, $this->eco_department_approval->ReadOnly);

			// eco_departmnet_comment
			$this->eco_departmnet_comment->SetDbValueDef($rsnew, $this->eco_departmnet_comment->CurrentValue, NULL, $this->eco_departmnet_comment->ReadOnly);
...
// Create page object
if (!isset($institutions_edit)) $institutions_edit = new cinstitutions_edit();

// Page init
$institutions_edit->Page_Init();

// Page main
$institutions_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$institutions_edit->Page_Render();
?>
...
var CurrentForm = finstitutionsedit = new ew_Form("finstitutionsedit", "edit");

// Validate form
finstitutionsedit.Validate = function() {
...
finstitutionsedit.Form_CustomValidate = 
...
finstitutionsedit.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Multi-Page
finstitutionsedit.MultiPage = new ew_MultiPage("finstitutionsedit");

// Dynamic selection lists
finstitutionsedit.Lists["x_institution_type"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
finstitutionsedit.Lists["x_institution_type"].Options = <?php echo json_encode($institutions_edit->institution_type->Options()) ?>;
finstitutionsedit.Lists["x_volunteering_type"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
finstitutionsedit.Lists["x_volunteering_type"].Options = <?php echo json_encode($institutions_edit->volunteering_type->Options()) ?>;
finstitutionsedit.Lists["x_nationality_type"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
finstitutionsedit.Lists["x_nationality_type"].Options = <?php echo json_encode($institutions_edit->nationality_type->Options()) ?>;
finstitutionsedit.Lists["x_current_emirate"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
finstitutionsedit.Lists["x_current_emirate"].Options = <?php echo json_encode($institutions_edit->current_emirate->Options()) ?>;
finstitutionsedit.Lists["x_admin_approval"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
finstitutionsedit.Lists["x_admin_approval"].Options = <?php echo json_encode($institutions_edit->admin_approval->Options()) ?>;
finstitutionsedit.Lists["x_forward_to_dep"] = {"LinkField":"x_userlevelid","Ajax":true,"AutoFill":false,"DisplayFields":["x_userlevelname","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"userlevels"};
finstitutionsedit.Lists["x_forward_to_dep"].Data = "<?php echo $institutions_edit->forward_to_dep->LookupFilterQuery(FALSE, "edit") ?>";
finstitutionsedit.Lists["x_eco_department_approval"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
finstitutionsedit.Lists["x_eco_department_approval"].Options = <?php echo json_encode($institutions_edit->eco_department_approval->Options()) ?>;
finstitutionsedit.Lists["x_security_approval"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
finstitutionsedit.Lists["x_security_approval"].Options = <?php echo json_encode($institutions_edit->security_approval->Options()) ?>;
...
<?php $institutions_edit->ShowPageHeader(); ?>
<?php
$institutions_edit->ShowMessage();
?>
<form name="finstitutionsedit" id="finstitutionsedit" class="<?php echo $institutions_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($institutions_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $institutions_edit->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="institutions">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<input type="hidden" name="modal" value="<?php echo intval($institutions_edit->IsModal) ?>">
<?php if (!$institutions_edit->IsMobileOrModal) { ?>
<div class="ewDesktop"><!-- desktop -->
<?php } ?>
<div class="ewMultiPage"><!-- multi-page -->
<div class="nav-tabs-custom" id="institutions_edit"><!-- multi-page .nav-tabs-custom -->
	<ul class="nav<?php echo $institutions_edit->MultiPages->NavStyle() ?>">
		<li<?php echo $institutions_edit->MultiPages->TabStyle("1") ?>><a href="#tab_institutions1" data-toggle="tab"><?php echo $institutions->PageCaption(1) ?></a></li>
		<li<?php echo $institutions_edit->MultiPages->TabStyle("2") ?>><a href="#tab_institutions2" data-toggle="tab"><?php echo $institutions->PageCaption(2) ?></a></li>
		<li<?php echo $institutions_edit->MultiPages->TabStyle("3") ?>><a href="#tab_institutions3" data-toggle="tab"><?php echo $institutions->PageCaption(3) ?></a></li>
		<li<?php echo $institutions_edit->MultiPages->TabStyle("4") ?>><a href="#tab_institutions4" data-toggle="tab"><?php echo $institutions->PageCaption(4) ?></a></li>
		<li<?php echo $institutions_edit->MultiPages->TabStyle("5") ?>><a href="#tab_institutions5" data-toggle="tab"><?php echo $institutions->PageCaption(5) ?></a></li>
		<li<?php echo $institutions_edit->MultiPages->TabStyle("6") ?>><a href="#tab_institutions6" data-toggle="tab"><?php echo $institutions->PageCaption(6) ?></a></li>
		<li<?php echo $institutions_edit->MultiPages->TabStyle("7") ?>><a href="#tab_institutions7" data-toggle="tab"><?php echo $institutions->PageCaption(7) ?></a></li>
		<li<?php echo $institutions_edit->MultiPages->TabStyle("8") ?>><a href="#tab_institutions8" data-toggle="tab"><?php echo $institutions->PageCaption(8) ?></a></li>
		<li<?php echo $institutions_edit->MultiPages->TabStyle("9") ?>><a href="#tab_institutions9" data-toggle="tab"><?php echo $institutions->PageCaption(9) ?></a></li>
	</ul>
	<div class="tab-content"><!-- multi-page .nav-tabs-custom .tab-content -->
		<div class="tab-pane<?php echo $institutions_edit->MultiPages->PageStyle("1") ?>" id="tab_institutions1"><!-- multi-page .tab-pane -->
<?php if ($institutions_edit->IsMobileOrModal) { ?>
<div class="ewEditDiv"><!-- page* -->
<?php } else { ?>
<table id="tbl_institutionsedit1" class="table table-striped table-bordered table-hover table-condensed ewDesktopTable"><!-- table* -->
<?php } ?>
<?php if ($institutions->institution_id->Visible) { // institution_id ?>
<?php if ($institutions_edit->IsMobileOrModal) { ?>
	<div id="r_institution_id" class="form-group">
		<label id="elh_institutions_institution_id" class="<?php echo $institutions_edit->LeftColumnClass ?>"><?php echo $institutions->institution_id->FldCaption() ?></label>
		<div class="<?php echo $institutions_edit->RightColumnClass ?>"><div<?php echo $institutions->institution_id->CellAttributes() ?>>
<span id="el_institutions_institution_id">
<span<?php echo $institutions->institution_id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $institutions->institution_id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="institutions" data-field="x_institution_id" data-page="1" name="x_institution_id" id="x_institution_id" value="<?php echo ew_HtmlEncode($institutions->institution_id->CurrentValue) ?>">
<?php echo $institutions->institution_id->CustomMsg ?></div></div>
	</div>
<?php } else { ?>
...
<input type="hidden" data-table="institutions" data-field="x_institution_id" data-page="1" name="x_institution_id" id="x_institution_id" value="<?php echo ew_HtmlEncode($institutions->institution_id->CurrentValue) ?>">
<?php echo $institutions->institution_id->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php } ?>
...
<?php if ($institutions_edit->IsMobileOrModal) { ?>
	<div id="r_full_name_ar" class="form-group">
		<label id="elh_institutions_full_name_ar" for="x_full_name_ar" class="<?php echo $institutions_edit->LeftColumnClass ?>"><?php echo $institutions->full_name_ar->FldCaption() ?></label>
		<div class="<?php echo $institutions_edit->RightColumnClass ?>"><div<?php echo $institutions->full_name_ar->CellAttributes() ?>>
...
<?php if ($institutions_edit->IsMobileOrModal) { ?>
	<div id="r_full_name_en" class="form-group">
		<label id="elh_institutions_full_name_en" for="x_full_name_en" class="<?php echo $institutions_edit->LeftColumnClass ?>"><?php echo $institutions->full_name_en->FldCaption() ?></label>
		<div class="<?php echo $institutions_edit->RightColumnClass ?>"><div<?php echo $institutions->full_name_en->CellAttributes() ?>>
...
<?php if ($institutions_edit->IsMobileOrModal) { ?>
	<div id="r_institution_type" class="form-group">
		<label id="elh_institutions_institution_type" class="<?php echo $institutions_edit->LeftColumnClass ?>"><?php echo $institutions->institution_type->FldCaption() ?></label>
		<div class="<?php echo $institutions_edit->RightColumnClass ?>"><div<?php echo $institutions->institution_type->CellAttributes() ?>>
...
<?php if ($institutions_edit->IsMobileOrModal) { ?>
	<div id="r_institutes_name" class="form-group">
		<label id="elh_institutions_institutes_name" for="x_institutes_name" class="<?php echo $institutions_edit->LeftColumnClass ?>"><?php echo $institutions->institutes_name->FldCaption() ?></label>
		<div class="<?php echo $institutions_edit->RightColumnClass ?>"><div<?php echo $institutions->institutes_name->CellAttributes() ?>>
...
<?php if ($institutions_edit->IsMobileOrModal) { ?>
	<div id="r_volunteering_type" class="form-group">
		<label id="elh_institutions_volunteering_type" class="<?php echo $institutions_edit->LeftColumnClass ?>"><?php echo $institutions->volunteering_type->FldCaption() ?></label>
		<div class="<?php echo $institutions_edit->RightColumnClass ?>"><div<?php echo $institutions->volunteering_type->CellAttributes() ?>>
...
<?php } ?>
		...
		<div class="tab-pane<?php echo $institutions_edit->MultiPages->PageStyle("2") ?>" id="tab_institutions2"><!-- multi-page .tab-pane -->
<?php if ($institutions_edit->IsMobileOrModal) { ?>
<div class="ewEditDiv"><!-- page* -->
<?php } else { ?>
<table id="tbl_institutionsedit2" class="table table-striped table-bordered table-hover table-condensed ewDesktopTable"><!-- table* -->
<?php } ?>
<?php if ($institutions->licence_no->Visible) { // licence_no ?>
<?php if ($institutions_edit->IsMobileOrModal) { ?>
	<div id="r_licence_no" class="form-group">
		<label id="elh_institutions_licence_no" for="x_licence_no" class="<?php echo $institutions_edit->LeftColumnClass ?>"><?php echo $institutions->licence_no->FldCaption() ?></label>
		<div class="<?php echo $institutions_edit->RightColumnClass ?>"><div<?php echo $institutions->licence_no->CellAttributes() ?>>
...
<?php if ($institutions_edit->IsMobileOrModal) { ?>
	<div id="r_trade_licence" class="form-group">
		<label id="elh_institutions_trade_licence" class="<?php echo $institutions_edit->LeftColumnClass ?>"><?php echo $institutions->trade_licence->FldCaption() ?></label>
		<div class="<?php echo $institutions_edit->RightColumnClass ?>"><div<?php echo $institutions->trade_licence->CellAttributes() ?>>
...
<?php if (@$_POST["fa_x_trade_licence"] == "0") { ?>
<input type="hidden" name="fa_x_trade_licence" id= "fa_x_trade_licence" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_trade_licence" id= "fa_x_trade_licence" value="1">
<?php } ?>
...
<?php if ($institutions_edit->IsMobileOrModal) { ?>
	<div id="r_tl_expiry_date" class="form-group">
		<label id="elh_institutions_tl_expiry_date" for="x_tl_expiry_date" class="<?php echo $institutions_edit->LeftColumnClass ?>"><?php echo $institutions->tl_expiry_date->FldCaption() ?></label>
		<div class="<?php echo $institutions_edit->RightColumnClass ?>"><div<?php echo $institutions->tl_expiry_date->CellAttributes() ?>>
...
<script type="text/javascript">
ew_CreateDateTimePicker("finstitutionsedit", "x_tl_expiry_date", {"ignoreReadonly":true,"useCurrent":false,"format":0});
</script>
<?php } ?>
</span>
...
<?php } ?>
		...
		<div class="tab-pane<?php echo $institutions_edit->MultiPages->PageStyle("3") ?>" id="tab_institutions3"><!-- multi-page .tab-pane -->
<?php if ($institutions_edit->IsMobileOrModal) { ?>
<div class="ewEditDiv"><!-- page* -->
<?php } else { ?>
<table id="tbl_institutionsedit3" class="table table-striped table-bordered table-hover table-condensed ewDesktopTable"><!-- table* -->
<?php } ?>
<?php if ($institutions->nationality_type->Visible) { // nationality_type ?>
<?php if ($institutions_edit->IsMobileOrModal) { ?>
	<div id="r_nationality_type" class="form-group">
		<label id="elh_institutions_nationality_type" class="<?php echo $institutions_edit->LeftColumnClass ?>"><?php echo $institutions->nationality_type->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $institutions_edit->RightColumnClass ?>"><div<?php echo $institutions->nationality_type->CellAttributes() ?>>
...
<?php if ($institutions_edit->IsMobileOrModal) { ?>
	<div id="r_nationality" class="form-group">
		<label id="elh_institutions_nationality" for="x_nationality" class="<?php echo $institutions_edit->LeftColumnClass ?>"><?php echo $institutions->nationality->FldCaption() ?></label>
		<div class="<?php echo $institutions_edit->RightColumnClass ?>"><div<?php echo $institutions->nationality->CellAttributes() ?>>
...
<?php if ($institutions_edit->IsMobileOrModal) { ?>
	<div id="r_visa_expiry_date" class="form-group">
		<label id="elh_institutions_visa_expiry_date" for="x_visa_expiry_date" class="<?php echo $institutions_edit->LeftColumnClass ?>"><?php echo $institutions->visa_expiry_date->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $institutions_edit->RightColumnClass ?>"><div<?php echo $institutions->visa_expiry_date->CellAttributes() ?>>
...
<script type="text/javascript">
ew_CreateDateTimePicker("finstitutionsedit", "x_visa_expiry_date", {"ignoreReadonly":true,"useCurrent":false,"format":0});
</script>
<?php } ?>
</span>
...
<?php if ($institutions_edit->IsMobileOrModal) { ?>
	<div id="r_unid" class="form-group">
		<label id="elh_institutions_unid" for="x_unid" class="<?php echo $institutions_edit->LeftColumnClass ?>"><?php echo $institutions->unid->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $institutions_edit->RightColumnClass ?>"><div<?php echo $institutions->unid->CellAttributes() ?>>
...
<?php if ($institutions_edit->IsMobileOrModal) { ?>
	<div id="r_visa_copy" class="form-group">
		<label id="elh_institutions_visa_copy" class="<?php echo $institutions_edit->LeftColumnClass ?>"><?php echo $institutions->visa_copy->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $institutions_edit->RightColumnClass ?>"><div<?php echo $institutions->visa_copy->CellAttributes() ?>>
...
<?php if (@$_POST["fa_x_visa_copy"] == "0") { ?>
<input type="hidden" name="fa_x_visa_copy" id= "fa_x_visa_copy" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_visa_copy" id= "fa_x_visa_copy" value="1">
<?php } ?>
...
<?php if ($institutions_edit->IsMobileOrModal) { ?>
	<div id="r_current_emirate" class="form-group">
		<label id="elh_institutions_current_emirate" for="x_current_emirate" class="<?php echo $institutions_edit->LeftColumnClass ?>"><?php echo $institutions->current_emirate->FldCaption() ?></label>
		<div class="<?php echo $institutions_edit->RightColumnClass ?>"><div<?php echo $institutions->current_emirate->CellAttributes() ?>>
...
<?php if ($institutions_edit->IsMobileOrModal) { ?>
	<div id="r_full_address" class="form-group">
		<label id="elh_institutions_full_address" for="x_full_address" class="<?php echo $institutions_edit->LeftColumnClass ?>"><?php echo $institutions->full_address->FldCaption() ?></label>
		<div class="<?php echo $institutions_edit->RightColumnClass ?>"><div<?php echo $institutions->full_address->CellAttributes() ?>>
...
<?php } ?>
		...
		<div class="tab-pane<?php echo $institutions_edit->MultiPages->PageStyle("4") ?>" id="tab_institutions4"><!-- multi-page .tab-pane -->
<?php if ($institutions_edit->IsMobileOrModal) { ?>
<div class="ewEditDiv"><!-- page* -->
<?php } else { ?>
<table id="tbl_institutionsedit4" class="table table-striped table-bordered table-hover table-condensed ewDesktopTable"><!-- table* -->
<?php } ?>
<?php if ($institutions->emirates_id_number->Visible) { // emirates_id_number ?>
<?php if ($institutions_edit->IsMobileOrModal) { ?>
	<div id="r_emirates_id_number" class="form-group">
		<label id="elh_institutions_emirates_id_number" for="x_emirates_id_number" class="<?php echo $institutions_edit->LeftColumnClass ?>"><?php echo $institutions->emirates_id_number->FldCaption() ?></label>
		<div class="<?php echo $institutions_edit->RightColumnClass ?>"><div<?php echo $institutions->emirates_id_number->CellAttributes() ?>>
...
<?php if ($institutions_edit->IsMobileOrModal) { ?>
	<div id="r_eid_expiry_date" class="form-group">
		<label id="elh_institutions_eid_expiry_date" for="x_eid_expiry_date" class="<?php echo $institutions_edit->LeftColumnClass ?>"><?php echo $institutions->eid_expiry_date->FldCaption() ?></label>
		<div class="<?php echo $institutions_edit->RightColumnClass ?>"><div<?php echo $institutions->eid_expiry_date->CellAttributes() ?>>
...
<script type="text/javascript">
ew_CreateDateTimePicker("finstitutionsedit", "x_eid_expiry_date", {"ignoreReadonly":true,"useCurrent":false,"format":0});
</script>
<?php } ?>
</span>
...
<?php if ($institutions_edit->IsMobileOrModal) { ?>
	<div id="r_emirates_id_copy" class="form-group">
		<label id="elh_institutions_emirates_id_copy" class="<?php echo $institutions_edit->LeftColumnClass ?>"><?php echo $institutions->emirates_id_copy->FldCaption() ?></label>
		<div class="<?php echo $institutions_edit->RightColumnClass ?>"><div<?php echo $institutions->emirates_id_copy->CellAttributes() ?>>
...
<?php if (@$_POST["fa_x_emirates_id_copy"] == "0") { ?>
<input type="hidden" name="fa_x_emirates_id_copy" id= "fa_x_emirates_id_copy" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_emirates_id_copy" id= "fa_x_emirates_id_copy" value="1">
<?php } ?>
...
<?php if ($institutions_edit->IsMobileOrModal) { ?>
	<div id="r_passport_number" class="form-group">
		<label id="elh_institutions_passport_number" for="x_passport_number" class="<?php echo $institutions_edit->LeftColumnClass ?>"><?php echo $institutions->passport_number->FldCaption() ?></label>
		<div class="<?php echo $institutions_edit->RightColumnClass ?>"><div<?php echo $institutions->passport_number->CellAttributes() ?>>
...
<?php if ($institutions_edit->IsMobileOrModal) { ?>
	<div id="r_passport_ex_date" class="form-group">
		<label id="elh_institutions_passport_ex_date" for="x_passport_ex_date" class="<?php echo $institutions_edit->LeftColumnClass ?>"><?php echo $institutions->passport_ex_date->FldCaption() ?></label>
		<div class="<?php echo $institutions_edit->RightColumnClass ?>"><div<?php echo $institutions->passport_ex_date->CellAttributes() ?>>
...
<script type="text/javascript">
ew_CreateDateTimePicker("finstitutionsedit", "x_passport_ex_date", {"ignoreReadonly":true,"useCurrent":false,"format":0});
</script>
<?php } ?>
</span>
...
<?php if ($institutions_edit->IsMobileOrModal) { ?>
	<div id="r_passport_copy" class="form-group">
		<label id="elh_institutions_passport_copy" class="<?php echo $institutions_edit->LeftColumnClass ?>"><?php echo $institutions->passport_copy->FldCaption() ?></label>
		<div class="<?php echo $institutions_edit->RightColumnClass ?>"><div<?php echo $institutions->passport_copy->CellAttributes() ?>>
...
<?php if (@$_POST["fa_x_passport_copy"] == "0") { ?>
<input type="hidden" name="fa_x_passport_copy" id= "fa_x_passport_copy" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_passport_copy" id= "fa_x_passport_copy" value="1">
<?php } ?>
...
<?php } ?>
		...
		<div class="tab-pane<?php echo $institutions_edit->MultiPages->PageStyle("5") ?>" id="tab_institutions5"><!-- multi-page .tab-pane -->
<?php if ($institutions_edit->IsMobileOrModal) { ?>
<div class="ewEditDiv"><!-- page* -->
<?php } else { ?>
<table id="tbl_institutionsedit5" class="table table-striped table-bordered table-hover table-condensed ewDesktopTable"><!-- table* -->
<?php } ?>
<?php if ($institutions->place_of_work->Visible) { // place_of_work ?>
<?php if ($institutions_edit->IsMobileOrModal) { ?>
	<div id="r_place_of_work" class="form-group">
		<label id="elh_institutions_place_of_work" for="x_place_of_work" class="<?php echo $institutions_edit->LeftColumnClass ?>"><?php echo $institutions->place_of_work->FldCaption() ?></label>
		<div class="<?php echo $institutions_edit->RightColumnClass ?>"><div<?php echo $institutions->place_of_work->CellAttributes() ?>>
...
<?php if ($institutions_edit->IsMobileOrModal) { ?>
	<div id="r_work_phone" class="form-group">
		<label id="elh_institutions_work_phone" for="x_work_phone" class="<?php echo $institutions_edit->LeftColumnClass ?>"><?php echo $institutions->work_phone->FldCaption() ?></label>
		<div class="<?php echo $institutions_edit->RightColumnClass ?>"><div<?php echo $institutions->work_phone->CellAttributes() ?>>
...
<?php if ($institutions_edit->IsMobileOrModal) { ?>
	<div id="r_mobile_phone" class="form-group">
		<label id="elh_institutions_mobile_phone" for="x_mobile_phone" class="<?php echo $institutions_edit->LeftColumnClass ?>"><?php echo $institutions->mobile_phone->FldCaption() ?></label>
		<div class="<?php echo $institutions_edit->RightColumnClass ?>"><div<?php echo $institutions->mobile_phone->CellAttributes() ?>>
...
<?php if ($institutions_edit->IsMobileOrModal) { ?>
	<div id="r_fax" class="form-group">
		<label id="elh_institutions_fax" for="x_fax" class="<?php echo $institutions_edit->LeftColumnClass ?>"><?php echo $institutions->fax->FldCaption() ?></label>
		<div class="<?php echo $institutions_edit->RightColumnClass ?>"><div<?php echo $institutions->fax->CellAttributes() ?>>
...
<?php if ($institutions_edit->IsMobileOrModal) { ?>
	<div id="r_pobbox" class="form-group">
		<label id="elh_institutions_pobbox" for="x_pobbox" class="<?php echo $institutions_edit->LeftColumnClass ?>"><?php echo $institutions->pobbox->FldCaption() ?></label>
		<div class="<?php echo $institutions_edit->RightColumnClass ?>"><div<?php echo $institutions->pobbox->CellAttributes() ?>>
...
<?php } ?>
		...
		<div class="tab-pane<?php echo $institutions_edit->MultiPages->PageStyle("6") ?>" id="tab_institutions6"><!-- multi-page .tab-pane -->
<?php if ($institutions_edit->IsMobileOrModal) { ?>
<div class="ewEditDiv"><!-- page* -->
<?php } else { ?>
<table id="tbl_institutionsedit6" class="table table-striped table-bordered table-hover table-condensed ewDesktopTable"><!-- table* -->
<?php } ?>
<?php if ($institutions->_email->Visible) { // email ?>
<?php if ($institutions_edit->IsMobileOrModal) { ?>
	<div id="r__email" class="form-group">
		<label id="elh_institutions__email" for="x__email" class="<?php echo $institutions_edit->LeftColumnClass ?>"><?php echo $institutions->_email->FldCaption() ?></label>
		<div class="<?php echo $institutions_edit->RightColumnClass ?>"><div<?php echo $institutions->_email->CellAttributes() ?>>
...
<?php if ($institutions_edit->IsMobileOrModal) { ?>
	<div id="r_password" class="form-group">
		<label id="elh_institutions_password" for="x_password" class="<?php echo $institutions_edit->LeftColumnClass ?>"><?php echo $institutions->password->FldCaption() ?></label>
		<div class="<?php echo $institutions_edit->RightColumnClass ?>"><div<?php echo $institutions->password->CellAttributes() ?>>
...
<?php } ?>
		...
		<div class="tab-pane<?php echo $institutions_edit->MultiPages->PageStyle("7") ?>" id="tab_institutions7"><!-- multi-page .tab-pane -->
<?php if ($institutions_edit->IsMobileOrModal) { ?>
<div class="ewEditDiv"><!-- page* -->
<?php } else { ?>
<table id="tbl_institutionsedit7" class="table table-striped table-bordered table-hover table-condensed ewDesktopTable"><!-- table* -->
<?php } ?>
<?php if ($institutions->admin_approval->Visible) { // admin_approval ?>
<?php if ($institutions_edit->IsMobileOrModal) { ?>
	<div id="r_admin_approval" class="form-group">
		<label id="elh_institutions_admin_approval" class="<?php echo $institutions_edit->LeftColumnClass ?>"><?php echo $institutions->admin_approval->FldCaption() ?></label>
		<div class="<?php echo $institutions_edit->RightColumnClass ?>"><div<?php echo $institutions->admin_approval->CellAttributes() ?>>
...
<?php if ($institutions_edit->IsMobileOrModal) { ?>
	<div id="r_admin_comment" class="form-group">
		<label id="elh_institutions_admin_comment" for="x_admin_comment" class="<?php echo $institutions_edit->LeftColumnClass ?>"><?php echo $institutions->admin_comment->FldCaption() ?></label>
		<div class="<?php echo $institutions_edit->RightColumnClass ?>"><div<?php echo $institutions->admin_comment->CellAttributes() ?>>
...
<?php if ($institutions_edit->IsMobileOrModal) { ?>
	<div id="r_forward_to_dep" class="form-group">
		<label id="elh_institutions_forward_to_dep" for="x_forward_to_dep" class="<?php echo $institutions_edit->LeftColumnClass ?>"><?php echo $institutions->forward_to_dep->FldCaption() ?></label>
		<div class="<?php echo $institutions_edit->RightColumnClass ?>"><div<?php echo $institutions->forward_to_dep->CellAttributes() ?>>
...
<?php } ?>
		...
		<div class="tab-pane<?php echo $institutions_edit->MultiPages->PageStyle("8") ?>" id="tab_institutions8"><!-- multi-page .tab-pane -->
<?php if ($institutions_edit->IsMobileOrModal) { ?>
<div class="ewEditDiv"><!-- page* -->
<?php } else { ?>
<table id="tbl_institutionsedit8" class="table table-striped table-bordered table-hover table-condensed ewDesktopTable"><!-- table* -->
<?php } ?>
...
<?php if ($institutions_edit->IsMobileOrModal) { ?>
	<div id="r_eco_department_approval" class="form-group">
		<label id="elh_institutions_eco_department_approval" class="<?php echo $institutions_edit->LeftColumnClass ?>"><?php echo $institutions->eco_department_approval->FldCaption() ?></label>
		<div class="<?php echo $institutions_edit->RightColumnClass ?>"><div<?php echo $institutions->eco_department_approval->CellAttributes() ?>>
...
<?php if ($institutions_edit->IsMobileOrModal) { ?>
	<div id="r_eco_departmnet_comment" class="form-group">
		<label id="elh_institutions_eco_departmnet_comment" for="x_eco_departmnet_comment" class="<?php echo $institutions_edit->LeftColumnClass ?>"><?php echo $institutions->eco_departmnet_comment->FldCaption() ?></label>
		<div class="<?php echo $institutions_edit->RightColumnClass ?>"><div<?php echo $institutions->eco_departmnet_comment->CellAttributes() ?>>
...
<?php } ?>
		...
		<div class="tab-pane<?php echo $institutions_edit->MultiPages->PageStyle("9") ?>" id="tab_institutions9"><!-- multi-page .tab-pane -->
<?php if ($institutions_edit->IsMobileOrModal) { ?>
<div class="ewEditDiv"><!-- page* -->
<?php } else { ?>
<table id="tbl_institutionsedit9" class="table table-striped table-bordered table-hover table-condensed ewDesktopTable"><!-- table* -->
<?php } ?>
<?php if ($institutions->security_approval->Visible) { // security_approval ?>
<?php if ($institutions_edit->IsMobileOrModal) { ?>
	<div id="r_security_approval" class="form-group">
		<label id="elh_institutions_security_approval" class="<?php echo $institutions_edit->LeftColumnClass ?>"><?php echo $institutions->security_approval->FldCaption() ?></label>
		<div class="<?php echo $institutions_edit->RightColumnClass ?>"><div<?php echo $institutions->security_approval->CellAttributes() ?>>
...
<?php if ($institutions_edit->IsMobileOrModal) { ?>
	<div id="r_security_comment" class="form-group">
		<label id="elh_institutions_security_comment" for="x_security_comment" class="<?php echo $institutions_edit->LeftColumnClass ?>"><?php echo $institutions->security_comment->FldCaption() ?></label>
		<div class="<?php echo $institutions_edit->RightColumnClass ?>"><div<?php echo $institutions->security_comment->CellAttributes() ?>>
...
<?php if (!$institutions_edit->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $institutions_edit->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("SaveBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $institutions_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
...
<script type="text/javascript">
finstitutionsedit.Init();
</script>
<?php
$institutions_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$institutions_edit->Page_Terminate();
?>
