...
function CurrentProjectID() {
	...
	return "{6A6E587E-A14E-4B9E-9243-D8812A8F089D}";
}
			...
			if ($rs = $UserTableConn->Execute($sSql)) {
				if (!$rs->EOF) {
					...
					if ($valid) {
						...
						$this->setSessionUserID($rs->fields('type')); // Load User ID
						if (is_null($rs->fields('type'))) {
							$this->setSessionUserLevelID(0);
						} else {
							$this->setSessionUserLevelID(intval($rs->fields('type'))); // Load User Level
						}
						$this->SetupUserLevel();
	...
	function LoadUserID() {
		...
		if (strval($this->CurrentUserID) == "") {
			...
			if ($RsUser = $UserTableConn->Execute($sSql)) {
				while (!$RsUser->EOF) {
					$this->AddUserID($RsUser->fields('type'));
					$RsUser->MoveNext();
				}
				$RsUser->Close();
			}
