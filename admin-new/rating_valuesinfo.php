...
//
// Table class for rating_values
//
class crating_values extends cTable {
	var $id;
	var $user_id;
	var $event_id;
	var $rated_by;
	...
	function __construct() {
		...
		$this->TableVar = 'rating_values';
		$this->TableName = 'rating_values';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`rating_values`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('rating_values', 'rating_values', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->Sortable = TRUE; // Allow sort
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// user_id
		$this->user_id = new cField('rating_values', 'rating_values', 'x_user_id', 'user_id', '`user_id`', '`user_id`', 201, -1, FALSE, '`user_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// event_id
		$this->event_id = new cField('rating_values', 'rating_values', 'x_event_id', 'event_id', '`event_id`', '`event_id`', 3, -1, FALSE, '`event_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		$this->event_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['event_id'] = &$this->event_id;

		// rated_by
		$this->rated_by = new cField('rating_values', 'rating_values', 'x_rated_by', 'rated_by', '`rated_by`', '`rated_by`', 3, -1, FALSE, '`rated_by`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->rated_by->Sortable = TRUE; // Allow sort
		$this->rated_by->UsePleaseSelect = TRUE; // Use PleaseSelect by default
		$this->rated_by->PleaseSelectText = $Language->Phrase("PleaseSelect"); // PleaseSelect text
		$this->rated_by->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['rated_by'] = &$this->rated_by;

		// rating_type
		$this->rating_type = new cField('rating_values', 'rating_values', 'x_rating_type', 'rating_type', '`rating_type`', '`rating_type`', 3, -1, FALSE, '`rating_type`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->rating_type->Sortable = TRUE; // Allow sort
		$this->rating_type->UsePleaseSelect = TRUE; // Use PleaseSelect by default
		$this->rating_type->PleaseSelectText = $Language->Phrase("PleaseSelect"); // PleaseSelect text
		$this->rating_type->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['rating_type'] = &$this->rating_type;

		// rating_value
		$this->rating_value = new cField('rating_values', 'rating_values', 'x_rating_value', 'rating_value', '`rating_value`', '`rating_value`', 3, -1, FALSE, '`rating_value`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->rating_value->Sortable = TRUE; // Allow sort
		$this->rating_value->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['rating_value'] = &$this->rating_value;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`rating_values`";
	}
...
function includeWhere() {
	if($_SESSION['event_id'] != ''){
		$whereEvent = " and event_id = ".$_SESSION['event_id'];
	}else{
		$whereEvent ="";
	}

	if( $_SESSION['user_id'] != ''){
		$whereUser = " and user_id = ".$_SESSION['user_id'];
	}else{
		$whereUser ="";
	}

	$this->_SqlWhere = " 1=1  $whereEvent $whereUser "; //$whereStatmnet;
}
/* wesam */	
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "rating_valuesedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "rating_valuesadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("rating_valuesview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("rating_valuesview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "rating_valuesadd.php?" . $this->UrlParm($parm);
		else
			$url = "rating_valuesadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("rating_valuesedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("rating_valuesadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("rating_valuesdelete.php", $this->UrlParm());
	}
	...
	function LoadListRowValues(&$rs) {
		...
		$this->event_id->setDbValue($rs->fields('event_id'));
		$this->rated_by->setDbValue($rs->fields('rated_by'));
		$this->rating_type->setDbValue($rs->fields('rating_type'));
		$this->rating_value->setDbValue($rs->fields('rating_value'));
	}
	...
	function RenderEditRow() {
		...
		$this->rating_value->EditValue = $this->rating_value->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->event_id->Exportable) $Doc->ExportCaption($this->event_id);
					if ($this->rated_by->Exportable) $Doc->ExportCaption($this->rated_by);
					if ($this->rating_type->Exportable) $Doc->ExportCaption($this->rating_type);
					if ($this->rating_value->Exportable) $Doc->ExportCaption($this->rating_value);
				} else {
					if ($this->id->Exportable) $Doc->ExportCaption($this->id);
					if ($this->user_id->Exportable) $Doc->ExportCaption($this->user_id);
					if ($this->event_id->Exportable) $Doc->ExportCaption($this->event_id);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->event_id->Exportable) $Doc->ExportField($this->event_id);
						if ($this->rated_by->Exportable) $Doc->ExportField($this->rated_by);
						if ($this->rating_type->Exportable) $Doc->ExportField($this->rating_type);
						if ($this->rating_value->Exportable) $Doc->ExportField($this->rating_value);
					} else {
						if ($this->id->Exportable) $Doc->ExportField($this->id);
						if ($this->user_id->Exportable) $Doc->ExportField($this->user_id);
						if ($this->event_id->Exportable) $Doc->ExportField($this->event_id);
