...
$registered_users_grid = NULL; // Initialize page object first

class cregistered_users_grid extends cregistered_users {
	...
	// Page object name
	var $PageObjName = 'registered_users_grid';

	// Grid form hidden field names
	var $FormName = 'fregistered_usersgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["registered_users"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_activity_id") && $objForm->HasValue("o_activity_id") && $this->activity_id->CurrentValue <> $this->activity_id->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_user_id") && $objForm->HasValue("o_user_id") && $this->user_id->CurrentValue <> $this->user_id->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_admin_approval") && $objForm->HasValue("o_admin_approval") && $this->admin_approval->CurrentValue <> $this->admin_approval->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_admin_comment") && $objForm->HasValue("o_admin_comment") && $this->admin_comment->CurrentValue <> $this->admin_comment->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_evaluation_rate") && $objForm->HasValue("o_evaluation_rate") && $this->evaluation_rate->CurrentValue <> $this->evaluation_rate->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function LoadDefaultValues() {
		...
		$this->evaluation_rate->OldValue = $this->evaluation_rate->CurrentValue;
	}
	...
	function LoadFormValues() {
		...
		$this->activity_id->setOldValue($objForm->GetValue("o_activity_id"));
	...
	function SetupMasterParms() {
		...
		if ($sMasterTblVar == "activities") {
			...
			if ($GLOBALS["activities"]->EventCancelled) $this->EventCancelled = TRUE;
		}
