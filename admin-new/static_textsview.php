...
$static_texts_view = NULL; // Initialize page object first

class cstatic_texts_view extends cstatic_texts {
	...
	// Page object name
	var $PageObjName = 'static_texts_view';
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["id"] <> "") {
				...
				$sReturnUrl = "static_textslist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "static_textslist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "static_textslist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($static_texts_view)) $static_texts_view = new cstatic_texts_view();

// Page init
$static_texts_view->Page_Init();

// Page main
$static_texts_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$static_texts_view->Page_Render();
?>
...
var CurrentForm = fstatic_textsview = new ew_Form("fstatic_textsview", "view");

// Form_CustomValidate event
fstatic_textsview.Form_CustomValidate = 
...
fstatic_textsview.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<?php if ($static_texts->Export == "") { ?>
<div class="ewToolbar">
<?php $static_texts_view->ExportOptions->Render("body") ?>
<?php
	foreach ($static_texts_view->OtherOptions as &$option)
		$option->Render("body");
?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php $static_texts_view->ShowPageHeader(); ?>
<?php
$static_texts_view->ShowMessage();
?>
<form name="fstatic_textsview" id="fstatic_textsview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($static_texts_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $static_texts_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="static_texts">
<input type="hidden" name="modal" value="<?php echo intval($static_texts_view->IsModal) ?>">
...
<?php echo $static_texts->id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $static_texts->label_text->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $static_texts->genuine_text_ar->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $static_texts->genuine_text_en->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php if ($static_texts->Export == "") { ?>
<script type="text/javascript">
fstatic_textsview.Init();
</script>
<?php } ?>
<?php
$static_texts_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($static_texts->Export == "") { ?>
...
<?php
$static_texts_view->Page_Terminate();
?>
