...
$user_attachments_grid = NULL; // Initialize page object first

class cuser_attachments_grid extends cuser_attachments {
	...
	// Page object name
	var $PageObjName = 'user_attachments_grid';

	// Grid form hidden field names
	var $FormName = 'fuser_attachmentsgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["user_attachments"];

		}
		$this->AddUrl = "user_attachmentsadd.php";
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_description") && $objForm->HasValue("o_description") && $this->description->CurrentValue <> $this->description->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_hours") && $objForm->HasValue("o_hours") && $this->hours->CurrentValue <> $this->hours->OldValue)
			return FALSE;
	...
	function LoadFormValues() {
		...
		$this->hours->setOldValue($objForm->GetValue("o_hours"));
	}
	...
	function RenderRow() {
		...
		$this->file->ViewCustomAttributes = "";
			...
			if (is_numeric($this->RowIndex) && !$this->EventCancelled) ew_RenderUploadField($this->file, $this->RowIndex);
